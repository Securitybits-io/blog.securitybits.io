(function($) {
  ...
  var Sidebar = function() {
    ...
    this.$blog = $('.post-bottom-bar, #main, .post-header-cover, .post, ' +
      '#bottom-bar .post-action-share')
      .add(this.$header)
  ...
  Sidebar.prototype = {
    ...
    swipeBlogToRight: function() {
      ...
      // Check if there is enough place for translating `#header .header-title` and
      // `#header .right-picture` regarding the size of `#header .header-title-link`
      ...
      var titleLinkWidth = this.$headerElements.titleLink.width();
      if (headerWidth - sidebarWidth - titleLinkWidth < 130) {
    ...
    swipeBlogToLeft: function() {
      ...
      // and prevent multiple click on the close button with `.processing` class
      if (self.$blog.hasClass('pushed') && !self.$blog.hasClass('processing')) {
        // Swipe the blog to the left
